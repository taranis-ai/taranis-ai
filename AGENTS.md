...
- NEVER add attribution to tools (e.g., "Generated by", "Co-authored-by:", or "Assisted-by:") to git commit messages. Legitimate human attribution and required third-party notices are unaffected.
- Do not edit the root `README.md` unless the user explicitly requests that edit. Put implementation, development, and deployment notes in the relevant scoped documentation instead.
- PRs changing root `README.md` or `.github/CODEOWNERS` require Ben's explicit review. Never use admin/review bypass permissions, dismiss reviews, or weaken branch protection to merge these changes. This also applies when using Ben's GitHub credentials or opening a PR under his account.
...
- Only update `README.md` if information already present in the file changed.
- Only update `deploy/README.md` regarding changes that require manual intervention while updating.
