...
- Centrifugo connection, heartbeat, and other control frames do not trigger resynchronization. Terminal disconnects stop reconnection; temporary failures retry eight times with jittered exponential backoff capped at 60 seconds and show one degraded notice after 15 seconds. A successful connection resets the retry budget. An internal disconnect response does not claim that page data changed when the connection recovers.
