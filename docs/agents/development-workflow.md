...
- Before preparing a PR, follow [Contributing](../../CONTRIBUTING.md) and use the [PR template](../../.github/pull_request_template.md). Include actual validation results and explain checks not run. After review changes, refresh the description, prompt disclosure, and validation evidence; rerun applicable checks, including the full signoff loop for feature changes.
