...
Malformed JSON or files that do not match the exported `version`/`data` shape should render a normal admin notification and must not leak a Flask traceback into the page.
...
Core creates only missing users, returns `users`/`count` for created users and `skipped_users`/`skipped_count` for duplicates, and always includes a user-facing `message` for successful or no-op imports.
...
Use `response.ok` in frontend import handling. A `requests.Response` with a 4xx status is falsey, so `if not response` incorrectly conflates core errors with no response.
