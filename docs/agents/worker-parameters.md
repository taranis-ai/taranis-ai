...
- Story clustering retains `BOT_ENDPOINT` and `BOT_API_KEY` only for persisted configuration compatibility. They are unused; worker `LLM_*` settings configure the direct library call, with `REQUESTS_TIMEOUT` overriding its timeout. See [Story Clustering](story-clustering.md).
