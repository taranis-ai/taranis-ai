...
Admin forms render schema-required parameters immediately in a responsive settings grid. Non-required parameters remain part of the submitted form but are visually grouped in a native `details` disclosure that is closed by default and reports how many optional keys are explicitly configured. Workers without required parameters explain that defaults are sufficient; selected workers with no parameters show a compact empty state. Dynamic HTMX type changes replace the complete shared parameter section, and the separate Assess create-from-URL dialog is not part of this shared form.
