...
- Story clustering and summarization retain their old endpoint/key parameters only for persisted configuration compatibility. They are unused; shared LLM Endpoints settings configure the direct library calls, with `REQUESTS_TIMEOUT` overriding the endpoint timeout. See [Story Clustering](story-clustering.md).
