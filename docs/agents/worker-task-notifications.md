...
MISP connector outcomes are persisted according to the completed operation rather than successful worker dispatch: synchronized stories and submitted proposals are successes, while a story that was not synchronized is a failure with a safe reason. Raw MISP exceptions and connector credentials are not exposed in the user task result.
