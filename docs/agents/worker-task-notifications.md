...
MISP connector outcomes are persisted according to the completed operation rather than successful worker dispatch: synchronized stories and submitted proposals are successes, while an entirely failed execution raises into the connector task's single failure path. Expected connector errors carry a curated public message and stable reason code; unexpected errors use a generic fallback, and raw exception text and connector credentials stay in server logs.
