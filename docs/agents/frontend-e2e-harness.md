...
- HTMX waits: `htmx_helpers.py` tracks requests through `htmx:finally:request`, after HTMX 4 processes swapped content. `test_htmx_helpers.py` uses the vendor bundle and a delayed outerHTML settle to cover clicking Share during card replacement; `test_user_assess` asserts the dialog opens without navigation. Do not gate settlement on swap/process timestamp ordering: swaps without new elements need no process event.
