...
## Pitfalls
RQ executes jobs in short-lived forked or spawned work-horse processes that terminate with `os._exit`; worker telemetry must flush after each job. Keep that flush bounded so an unavailable backend cannot stall a completed job indefinitely. The SDK's batch-processor flush timeout is not enforced by every release, so bound the exporters' network timeouts too. Do not initialize exporter threads only in the long-lived RQ polling process. Keep metric labels bounded to queue, task function, and status. Do not place credentials, arguments, job results, source content, or report content in span attributes or metric labels.
