...
## Pitfalls
RQ executes jobs in short-lived forked or spawned work-horse processes that terminate with `os._exit`; worker telemetry must flush after each job. Trace and metric exporters each receive half of the one-second combined flush budget. The SDK's batch-processor flush timeout is not enforced by every release, so keep the exporters' network timeouts aligned with that budget. Do not initialize exporter threads only in the long-lived RQ polling process. Keep metric labels bounded to queue, task function, and status. Do not place credentials, arguments, job results, source content, or report content in span attributes or metric labels.
