...
- Both services force Secure JWT/CSRF and session cookies outside DEBUG, and use SameSite=Lax. Access JWT/session cookies are HttpOnly; CSRF cookies remain JavaScript-readable. See [Browser Security](browser-security.md) for headers and deployment requirements.
...
- Frontend HTTP sessions are scoped to one request and bearer token; explicitly supplied tokens override browser cookies. Download resources have a separate lifetime. See [HTTP Client Lifetimes](http-clients.md).
