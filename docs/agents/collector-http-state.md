...
Browser-backed article fetches use the main navigation response's Last-Modified header as the news item's published-date fallback. Invalid Last-Modified values are ignored in all fetch modes. RSS published dates use the first parseable, non-blank value from the supported entry fields, then the linked article date, the channel's `lastBuildDate`, and finally the feed response's HTTP Last-Modified validator. Secondary browser responses do not update the primary resource's persisted HTTP validators.
