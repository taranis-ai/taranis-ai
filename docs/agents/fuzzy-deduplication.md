...
- Results contain counts and affected IDs, including committed progress when a later item or post-commit scheduling fails. The worker retains these IDs from error responses and dispatches post-collection bots for them before reporting collection failure; this does not retry ingestion. Preserve `All news items were skipped` for unchanged/stale-only runs, which collectors translate to `NOT_MODIFIED`. Invalid payloads are errors, not skipped duplicates.
...
Core worker-pipeline tests cover identity, history, stale retries, scope, threshold boundaries, and invalid batches. The partial-batch test uses savepoints inside an explicit SQLite transaction to exercise per-item commits and rollbacks without leaking fixture data. Committed changes notify Assess and invalidate its caches even when the batch fails. Worker collector tests cover validator scope and post-collection dispatch for updates and partial failures. Frontend `test_collected_article_updates_and_grouping` verifies the displayed diff and two retained grouped URLs. Follow the normal full feature signoff workflow.
