...
- Worker captures collection start before fetching and forwards it as `collected`. Core keeps `collection_seen_at` separately, skipping older/equal deliveries without revisions. `collected` remains the first collection date; `updated` also changes for non-collection edits, so neither orders accepted fetches. Unchanged newer deliveries advance this watermark without changing item `updated`. Publisher version order cannot be inferred from run timestamps.
