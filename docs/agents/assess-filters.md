...
- Successful clustering replaces the saved Assess selection with the first story in the submitted dialog order before the list swap; merged-away IDs must not survive into the next action. Failed clustering preserves selection and the open dialog. Repeated clustering is covered in `test_assess` in `test_e2e_workflow.py`.
