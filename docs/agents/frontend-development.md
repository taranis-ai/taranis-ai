...
- Table actions returning a full container use outerHTML; Analyze actions target the outer `#report` wrapper. Notification-only actions must not receive table markup.
- Keep Report Type required in the shared Analyze/Assess report form so invalid submissions preserve unsaved input.
