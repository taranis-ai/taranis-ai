...
Renovate performs weekly dependency and lockfile updates, which follow the same seven-day exclusion policy.
Pull requests and local push signoff run `uv audit --locked` against every component's dependency graph so known vulnerabilities and stale lockfiles fail validation.
