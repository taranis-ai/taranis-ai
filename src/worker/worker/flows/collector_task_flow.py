...
def execute_collector(source: dict, manual: bool = False):
    ...
    try:
        ...
        task_description = (
            f"Collect: source '{source.get('name')}' with id {source.get('id')} "
            f"using collector: '{collector_instance.name}'"
        )
...
def collector_task_flow(request: CollectorTaskRequest):
    ...
    try:
        ...
        return {
            "message": f"Collection from source {request.source_id} completed",
            "result": result,
            "status": "success"
        }
