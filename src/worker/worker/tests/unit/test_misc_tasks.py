...
def test_reconcile_task_failures_calls_core_and_persists_success(monkeypatch):
    recorded = {}

    class DummyApi:
        def reconcile_task_failures(self):
            return {"message": "Task reconciliation completed", "reconciled": 2}
    ...
    payload = misc_tasks.reconcile_task_failures()

    assert payload == {"message": "Task reconciliation completed", "reconciled": 2}
    assert recorded["payload"] == {
        "id": DummyJob.id,
        "task": "reconcile_task_failures",
        "worker_id": "reconcile_task_failures",
        "worker_type": "reconcile_task_failures",
        "result": {
            "message": "Task reconciliation completed",
            "reason": None,
            "retryable": False,
            "data": {"message": "Task reconciliation completed", "reconciled": 2},
        },
        "status": "SUCCESS",
    }


def test_cleanup_task_history_calls_core_and_persists_success(monkeypatch):
    recorded = {}

    class DummyApi:
        def cleanup_task_history(self):
            return {"message": "Task history cleanup completed", "deleted": 4, "cutoff": "2026-06-09T00:00:00"}
    ...
    payload = misc_tasks.cleanup_task_history()

    assert payload == {"message": "Task history cleanup completed", "deleted": 4, "cutoff": "2026-06-09T00:00:00"}
    assert recorded["payload"] == {
        "id": DummyJob.id,
        "task": "cleanup_task_history",
        "worker_id": "cleanup_task_history",
        "worker_type": "cleanup_task_history",
        "result": {
            "message": "Task history cleanup completed",
            "reason": None,
            "retryable": False,
            "data": {"message": "Task history cleanup completed", "deleted": 4, "cutoff": "2026-06-09T00:00:00"},
        },
        "status": "SUCCESS",
    }
