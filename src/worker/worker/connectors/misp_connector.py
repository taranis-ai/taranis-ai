...
class MISPConnector(BaseMispBuilder):
    ...
    def add_misp_event(self, misp: PyMISP, story: dict) -> MISPEvent | None:
        event = self.create_misp_event(story, self.sharing_group_id, self.distribution)
    ...
    def _create_event(self, story_prepared, misp_event_uuid, existing_event):
        result = self.create_misp_event(story_prepared, self.sharing_group_id, self.distribution)
