...
class AssetGroup(Resource):
    ...
    def put(self, group_id):
        asset.AssetGroup.update(
            auth_manager.get_user_from_jwt(), group_id, request.json
        )
...
class NotificationTemplates(Resource):
    ...
    def get(self):
        ...
        if "search" in request.args and request.args["search"]:
            search = request.args["search"]
        return notification_template.NotificationTemplate.get_all_json(
            auth_manager.get_user_from_jwt(), search
        )

    @auth_required("MY_ASSETS_CONFIG")
    def post(self):
        return "", notification_template.NotificationTemplate.add(
            auth_manager.get_user_from_jwt(), request.json
        )
...
class NotificationTemplate(Resource):
    ...
    def put(self, template_id):
        notification_template.NotificationTemplate.update(
            auth_manager.get_user_from_jwt(), template_id, request.json
        )

    @auth_required("MY_ASSETS_CONFIG")
    def delete(self, template_id):
        return notification_template.NotificationTemplate.delete(
            auth_manager.get_user_from_jwt(), template_id
        )
...
class Assets(Resource):
    ...
    def get(self, group_id):
        ...
        return asset.Asset.get_all_json(
            auth_manager.get_user_from_jwt(), group_id, search, sort, vulnerable
        )

    @auth_required("MY_ASSETS_CREATE")
    def post(self, group_id):
        return "", asset.Asset.add(
            auth_manager.get_user_from_jwt(), group_id, request.json
        )
...
class Asset(Resource):
    ...
    def put(self, group_id, asset_id):
        asset.Asset.update(
            auth_manager.get_user_from_jwt(), group_id, asset_id, request.json
        )
...
def initialize(api):
    ...
    api.add_resource(
        NotificationTemplates, "/api/v1/my-assets/asset-notification-templates"
    )
    api.add_resource(
    ...
    api.add_resource(
        Asset, "/api/v1/my-assets/asset-groups/<string:group_id>/assets/<int:asset_id>"
    )
