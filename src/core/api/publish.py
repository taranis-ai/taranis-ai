...
from managers import auth_manager, presenters_manager, publishers_manager
...
class Products(Resource):
    ...
    def get(self):
        ...
        return product.Product.get_json(
            filter, offset, limit, auth_manager.get_user_from_jwt()
        )
        ...
        if "jwt" in request.args:
            ...
            if user is not None:
                ...
                if "PUBLISH_ACCESS" in permissions:
                    if product_type.ProductType.allowed_with_acl(
                        product_id, user, False, True, False
                    ):
                        product_data, status_code = presenters_manager.generate_product(
                            product_id
                        )
                        if status_code == 200:
                            return Response(
                        ...
                        logger.store_auth_error_activity(
                            "Unauthorized access attempt to Product Type"
                        )
                else:
...
def initialize(api):
    ...
    api.add_resource(
        ProductsOverview, "/api/v1/publish/products/<int:product_id>/overview"
    )
    api.add_resource(
        PublishProduct,
