...
from managers import auth_manager, sse_manager
...
class OSINTSourceGroupsList(Resource):
    @auth_required("ASSESS_ACCESS")
    def get(self):
        return osint_source.OSINTSourceGroup.get_list_json(
            auth_manager.get_user_from_jwt(), True
        )
...
class ManualOSINTSources(Resource):
    @auth_required(["ASSESS_ACCESS"])
    def get(self):
        return osint_source.OSINTSource.get_all_manual_json(
...
class NewsItemsByGroup(Resource):
    ...
    def get(self, group_id):
        ...
        return news_item.NewsItemAggregate.get_by_group_json(
...
class NewsItem(Resource):
    ...
    def put(self, item_id):
        user = auth_manager.get_user_from_jwt()
        response, osint_source_ids, code = news_item.NewsItem.update(
            item_id, request.json, user.id
        )
        sse_manager.news_items_updated()
...
class NewsItemAggregate(Resource):
    ...
    def put(self, aggregate_id):
        user = auth_manager.get_user_from_jwt()
        response, osint_source_ids, code = news_item.NewsItemAggregate.update(
            aggregate_id, request.json, user
        )
        sse_manager.news_items_updated()
        if len(osint_source_ids) > 0:
            sse_manager.remote_access_news_items_updated(osint_source_ids)
...
class GroupAction(Resource):
    ...
    def put(self):
        ...
        if request.json["action"] == "DELETE":
            response, code = news_item.NewsItemAggregate.group_action_delete(
                request.json, user
            )
            sse_manager.news_items_updated()
            return response, code
        else:
            response, osint_source_ids, code = news_item.NewsItemAggregate.group_action(
                request.json, user
            )
            sse_manager.news_items_updated()
            if len(osint_source_ids) > 0:
        ...
        if "jwt" in request.args:
            ...
            if user is not None:
                ...
                if "ASSESS_ACCESS" in permissions:
                    attribute_mapping = news_item.NewsItemDataNewsItemAttribute.find(
                        attribute_id
                    )
                    need_check = False
                    ...
                    if (
                        ...
                        logger.store_user_activity(
                            user, "ASSESS_ACCESS", str({"file": attribute.value})
                        )
                        return send_file(
                        ...
                        logger.store_auth_error_activity(
                            "Unauthorized access attempt to News Item Data"
                        )
                else:
...
def initialize(api):
    ...
    api.add_resource(
        NewsItemAggregate, "/api/v1/assess/news-item-aggregates/<int:aggregate_id>"
    )
