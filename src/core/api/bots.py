...
class NewsItemData(Resource):
    ...
    def get(self):
        try:
            limit = datetime.strftime(
                datetime.now() - timedelta(weeks=1), "%d.%m.%Y - %H:%M"
            )
...
class UpdateNewsItemAttributes(Resource):
    ...
    def put(self, news_item_data_id):
        news_item.NewsItemData.update_news_item_attributes(
            news_item_data_id, request.json
        )
...
class GetNewsItemsAggregate(Resource):
    ...
    def get(self, group_id):
        ...
        resp_str = news_item.NewsItemAggregate.get_news_items_aggregate(
...
class GetDefaultNewsItemsAggregate(Resource):
    ...
    def get(self):
        ...
        resp_str = news_item.NewsItemAggregate.get_news_items_aggregate(
            "3c8ef48d-411a-42ce-a0d6-7fd971676ff3", request.json
        )
        return json.loads(resp_str)
...
class Entries(Resource):
    ...
    def put(self, category_id, entry_name):
        return word_list.WordListEntry.update_word_list_entries(
            category_id, entry_name, request.json
        )
