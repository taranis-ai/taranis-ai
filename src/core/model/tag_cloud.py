...
class TagCloud(db.Model):
    ...
    def add_tag_clouds(cls, tag_clouds):
        for tag_cloud in tag_clouds:
            word = TagCloud.query.filter_by(
                word=tag_cloud.word, collected=tag_cloud.collected
            ).first()
            if word is not None:
    ...
    def get_grouped_words(cls, tag_cloud_day):
        day_filter = (
            datetime.datetime.now() - datetime.timedelta(days=tag_cloud_day)
        ).date()
        stopwords = WordListEntry.stopwords_subquery()
        grouped_words = (
            db.session.query(
                TagCloud.word, label("word_quantity", func.sum(TagCloud.word_quantity))
            )
    ...
    def generate_tag_cloud_words(cls, news_item_data):
        ...
        news_items_words = (
            news_items_title_words + news_items_review_words + news_items_content_words
        )
