...
def handle_task_specific_result(task_id: str, result: dict | str, status: str):
    ...
    elif task_id == "connector_report-to-stix":
        if stix_report := result.get("render_result"):
            ReportItem.update_render_for_id(result.get("report_id"), stix_report)
            logger.info("STIX report preparing for download")

        else:
            logger.error("No STIX report generated by connector")
