...
class ReportItem(MethodView):
    ...
    def post(self):
        try:
            ...
            if status != 200 or not isinstance(new_report_item, report_item.ReportItem):
                return jsonify(new_report_item), status
        ...
        return jsonify({"message": "New report item created", "id": new_report_item.id, "report": new_report_item.to_detail_dict()}), status
    ...
    def put(self, report_item_id: str | None = None):
        ...
        return jsonify({"message": "Report item updated", "id": updated_report.get("id"), "report": updated_report}), status
...
class ReportItemRevisions(MethodView):
    ...
    def get(self, report_item_id: str):
        ...
        if access_status != 200:
            return jsonify(access_response), access_status
