...
class KeycloakAuthenticator(BaseAuthenticator):
    def authenticate(self, credentials):
        ...
        response = post(
            url=environ.get("TARANIS_NG_KEYCLOAK_INTERNAL_URL")
            + "auth/realms/taranis_ng/protocol/openid-connect/token",
            data={
                "grant_type": "authorization_code",
                "code": request.args["code"],  # code from url
                "redirect_uri": "/".join(request.headers.get("Referer").split("/")[0:3])
                + "/login"
        ...
        try:
            ...
            logger.store_auth_error_activity(
                "Keycloak returned an unexpected response."
            )
