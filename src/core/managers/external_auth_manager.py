...
def get_keycloak_admin():
    return KeycloakAdmin(server_url=os.getenv('KEYCLOAK_SERVER_URL'),
...
def create_user(user_data):
    if keycloak_user_management_enabled():
        ...
        keycloak_admin.create_user({'username': user_data['username'],
        ...
        if 'password' in user_data and user_data['password'] or original_username != user_data['username']:
            ...
            if keycloak_user_id is not None:
                ...
                if 'password' in user_data and user_data['password']:
                    keycloak_admin.set_user_password(user_id=keycloak_user_id, password=user_data['password'],
                                                     temporary=False)
