...
class TestWorkerTaskResults:
    ...
    def test_task_errors_endpoint_returns_persisted_manual_collector_failure(self, client, auth_header, app):
        from core.model.task import Task
        ...
        try:
            response = client.get(
                f"{self.base_uri}/errors?scope=current&category=collector&search={marker}&page=1&limit=5&order=last_run_desc",
                headers=auth_header,
            )
            ...
            fallback_response = client.get(
                f"{self.base_uri}/errors?scope=invalid&category=invalid&search={marker}&page=-1&limit=-1",
                headers=auth_header,
            )
            assert fallback_response.status_code == 200
            assert fallback_response.get_json()["total_count"] == 1
        finally:
            with app.app_context():
                if Task.get(task_id):
                    Task.delete(task_id)
