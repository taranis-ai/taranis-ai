...
class TestAdminApi(BaseTest):
    ...
    def test_settings_update(self, client, auth_header):
        ...
        assert response_settings["settings"]["completed_onboarding_tours"] == {}

    def test_settings_update_marks_onboarding_tour_completed(self, client, auth_header):
        response = self.assert_put_ok(
            client,
            "settings",
            {"settings": {"completed_onboarding_tours": {"admin_welcome_v1": "dismissed"}}},
            auth_header,
        )

        assert response.get_json()["settings"]["completed_onboarding_tours"]["admin_welcome_v1"] == "dismissed"
