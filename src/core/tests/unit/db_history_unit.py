...
class TestVersioning(AssertsCompiledSQL):
    ...
    def test_index_naming(self, constraint_type):
        ...
        if constraint_type.unique_constraint_naming_conv or constraint_type.fk_constraint_naming_conv:
        ...
        if constraint_type.fk_constraint or constraint_type.fk_constraint_naming_conv or constraint_type.fk_constraint_explicit_name:
        ...
        class SomeClass(Versioned, self.Base):
            ...
            elif constraint_type.unique_constraint or constraint_type.unique_constraint_naming_conv:
            ...
            elif constraint_type.fk_constraint or constraint_type.fk_constraint_naming_conv:
                __table_args__ = (ForeignKeyConstraint([x], [Related.id]),)
            elif constraint_type.fk_constraint_explicit_name:
                __table_args__ = (ForeignKeyConstraint([x], [Related.id], name="my_fk"),)
            else:
                constraint_type.fail()

        eq_(
            set(idx.name + "_history" for idx in SomeClass.__table__.indexes),
            set(idx.name for idx in SomeClass.__history_mapper__.local_table.indexes),
        )
        self.create_tables()
    ...
    def test_discussion_9546(self):
        ...
        class ThingLocal(Versioned, self.Base):
            ...
            id = Column(Integer, ForeignKey(ThingExternal.id), primary_key=True)
        ...
        class Thing(self.Base):
            ...
            version = column_property(ThingExternal.version, ThingLocal.version)
    ...
    def test_joined_inheritance_multilevel(self):
        ...
        eq_(
            ssc,
            SubSubClass(name="ss1", subdata1="sd11", subdata2="sd22", version=2),
        )
    ...
    def test_joined_inheritance_changed(self):
        ...
        actual_changed_base = sess.scalar(select(BaseClass.__history_mapper__.local_table.c.changed))
        actual_changed_sub = sess.scalar(select(SubClass.__history_mapper__.local_table.c.changed))
