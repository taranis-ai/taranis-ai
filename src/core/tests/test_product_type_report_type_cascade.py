...
class TestProductTypeDeletionCascade:
    ...
    def sample_report_type(self, app):
        ...
        with app.app_context():
            report_type = ReportItemType(
                ...
                description="A test report type for cascade testing"
            )
    ...
    def test_initial_relationship_setup(self, app, sample_product_type, sample_report_type):
        """Test that the n:m relationship is properly established"""
        with app.app_context():
            # Get fresh objects from the database
            product_type = ProductType.get(sample_product_type.id)
            report_type = ReportItemType.get(sample_report_type.id)
            
            assert product_type is not None, f"ProductType {sample_product_type.id} not found"
            assert report_type is not None, f"ReportItemType {sample_report_type.id} not found"
            
            # Verify the relationship exists from ProductType side by checking IDs
            report_type_ids = [rt.id for rt in product_type.report_types]
            assert report_type.id in report_type_ids
            
            # Verify the association table entry exists
            association = db.session.query(ProductTypeReportType).filter_by(
                product_type_id=product_type.id,
                report_item_type_id=report_type.id
            ).first()
            assert association is not None

    def test_multiple_product_types_same_report_type(self, app, sample_product_type, additional_product_type, sample_report_type):
        """Test that multiple ProductTypes can reference the same ReportItemType"""
        with app.app_context():
            # Store the IDs for later use
            pt1_id = sample_product_type.id
            pt2_id = additional_product_type.id
            rt_id = sample_report_type.id
            
            # Query fresh objects from the database
            pt1 = db.session.query(ProductType).filter_by(id=pt1_id).first()
            pt2 = db.session.query(ProductType).filter_by(id=pt2_id).first()
            rt = db.session.query(ReportItemType).filter_by(id=rt_id).first()
            
            # Verify both product types have the same report type by checking IDs
            product_type1_report_ids = [rt_item.id for rt_item in pt1.report_types]
            product_type2_report_ids = [rt_item.id for rt_item in pt2.report_types]
            
            assert rt.id in product_type1_report_ids
            assert rt.id in product_type2_report_ids
            
            # Verify both association table entries exist
            associations = db.session.query(ProductTypeReportType).filter_by(
                report_item_type_id=rt.id
            ).all()
            assert len(associations) == 2
            
            product_type_ids = {assoc.product_type_id for assoc in associations}
            assert product_type_ids == {pt1.id, pt2.id}
    ...
    def test_product_type_deletion_cascade_behavior(self, app, sample_product_type, additional_product_type, sample_report_type):
        ...
        with app.app_context():
            ...
            initial_associations = db.session.query(ProductTypeReportType).filter_by(
            ...
            remaining_associations = db.session.query(ProductTypeReportType).filter_by(
                report_item_type_id=report_type_id
            ).all()
            ...
            deleted_associations = db.session.query(ProductTypeReportType).filter_by(
                product_type_id=product_type_to_delete_id
            ).all()
    ...
    def test_database_cascade_constraint_analysis(self, app):
        ...
        with app.app_context():
            ...
            product_type = ProductType(
                title="Constraint Analysis Product Type",
                type=PRESENTER_TYPES.TEXT_PRESENTER,
                report_types=[report_type.id]
            )
            db.session.add(product_type)
            db.session.commit()
            ...
            remaining_association = db.session.query(ProductTypeReportType).filter_by(
                product_type_id=product_type_id,
                report_item_type_id=report_type_id
            ).first()
    ...
    def test_product_type_deletion_removes_associations(self, app, sample_product_type, additional_product_type, sample_report_type):
        ...
        with app.app_context():
            ...
            initial_associations = db.session.query(ProductTypeReportType).filter_by(
                report_item_type_id=original_report_type_id
            ).all()
            assert len(initial_associations) == 2
    ...
    def test_current_cascade_behavior_summary(self, app):
        ...
        with app.app_context():
            ...
            delete_associations = db.session.query(ProductTypeReportType).filter_by(
                product_type_id=delete_id
            ).all()
            keep_associations = db.session.query(ProductTypeReportType).filter_by(
                product_type_id=keep_id
            ).all()
            ...
            if status_code == 200:
                ...
                remaining_delete_associations = db.session.query(ProductTypeReportType).filter_by(
                    product_type_id=delete_id
                ).all()
                remaining_keep_associations = db.session.query(ProductTypeReportType).filter_by(
                    product_type_id=keep_id
                ).all()
