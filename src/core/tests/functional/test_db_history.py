...
class TestDbHistory:
    def test_story_history(self, db_global_session, full_story):  # assumes a fixture that gives a working session
        ...
        assert db_global_session.query(StoryHistory).filter(StoryHistory.version == 1).count() == 1
        assert db_global_session.query(StoryHistory).filter(StoryHistory.version == 1).first().title == "Test Aggregate"

        # Should NOT create a history row for the current state
        current = Story.get(story.get("id", ""))
        assert current.title == "Second title"

    def test_story_multiple_updates_history(self, db_global_session, full_story):
        ...
        total_history_count = db_global_session.query(StoryHistory).filter(StoryHistory.id == story_id).count()
        test_logger.debug(f"Total history entries: {total_history_count}")

        # Debug: Let's see what's actually in the history table
        all_history = db_global_session.query(StoryHistory).filter(StoryHistory.id == story_id).order_by(StoryHistory.version).all()
        ...
        version_1 = db_global_session.query(StoryHistory).filter(StoryHistory.id == story_id, StoryHistory.version == 1).first()
        ...
        version_2 = db_global_session.query(StoryHistory).filter(StoryHistory.id == story_id, StoryHistory.version == 2).first()
        ...
        version_3 = db_global_session.query(StoryHistory).filter(StoryHistory.id == story_id, StoryHistory.version == 3).first()
        ...
        version_4 = db_global_session.query(StoryHistory).filter(StoryHistory.id == story_id, StoryHistory.version == 4).first()
    ...
    def test_story_attribute_update_history(self, db_global_session, full_story):
    ...
    def test_single_transaction_behavior(self, db_global_session, full_story):
        ...
        StoryHistory = Story.__history_mapper__.class_
        creation_history_count = db_global_session.query(StoryHistory).filter(StoryHistory.id == story_id).count()
