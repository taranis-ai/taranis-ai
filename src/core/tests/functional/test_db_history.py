...
class TestDbHistory:
    ...
    def test_story_multiple_updates_history(self, session, full_story):
        ...
        for i, history_entry in enumerate(all_history):
            test_logger.debug(f"History entry {i+1}: version={history_entry.version}, title='{history_entry.title}', description='{history_entry.description}'")
        
        assert total_history_count == 4, f"Expected 4 history entries, got {total_history_count}"
        
        # Check version 1 (original state)
        version_1 = session.query(StoryHistory).filter(
            StoryHistory.id == story_id,
            StoryHistory.version == 1
        ).first()
        ...
        version_2 = session.query(StoryHistory).filter(
            StoryHistory.id == story_id,
            StoryHistory.version == 2
        ).first()
        ...
        version_3 = session.query(StoryHistory).filter(
            StoryHistory.id == story_id,
            StoryHistory.version == 3
        ).first()
        ...
        version_4 = session.query(StoryHistory).filter(
            StoryHistory.id == story_id,
            StoryHistory.version == 4
        ).first()
