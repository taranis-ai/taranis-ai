...
class TestPresenterFlow:
    ...
    def test_pdf_presenter_flow_creates_render(self, api_client: TaranisAPIClient, test_data: TestData):
        ...
        flow_run_id = api_client.trigger_presenter_flow(
            product_id=test_data.product_id,
            presenter_id=test_data.presenter_id
        )
        ...
        if final_renders:
            ...
            print(f"Render created successfully:")
    ...
    def test_presenter_flow_with_invalid_product(self, api_client: TaranisAPIClient, test_data: TestData):
        ...
        with pytest.raises(Exception) as exc_info:
            api_client.trigger_presenter_flow(
...
class TestPublisherFlow:
    ...
    def test_ftp_publisher_flow_creates_publication(self, api_client: TaranisAPIClient, test_data: TestData):
        ...
        presenter_flow_id = api_client.trigger_presenter_flow(
            product_id=test_data.product_id,
            presenter_id=test_data.presenter_id
        )
        ...
        flow_run_id = api_client.trigger_publisher_flow(
            product_id=test_data.product_id,
        ...
        if final_publications:
            ...
            assert latest_publication.get("status") == "PUBLISHED", f"Publication status is {latest_publication.get('status')}, expected PUBLISHED"
            assert latest_publication.get("published_at"), "Publication missing published_at timestamp"
            
            print(f"Publication created successfully:")
...
class TestConnectorFlow:
    ...
    def test_misp_connector_flow_pushes_news_items(self, api_client: TaranisAPIClient, test_data: TestData):
        ...
        flow_run_id = api_client.trigger_connector_flow(
            connector_id=test_data.connector_id,
            story_ids=test_data.news_item_ids
        )
        ...
        assert len(finally_pushed) > len(initially_pushed), f"No additional items were pushed. Expected > {len(initially_pushed)}, got {len(finally_pushed)}"
...
class TestFlowIntegration:
    ...
    def test_complete_content_pipeline(self, api_client: TaranisAPIClient, test_data: TestData, flow_runner):
        ...
        presenter_flow_id = flow_runner("presenter",
                                       product_id=test_data.product_id,
                                       presenter_id=test_data.presenter_id,
                                       timeout=180)
        ...
        publisher_flow_id = flow_runner("publisher",
                                       product_id=test_data.product_id,
                                       publisher_id=test_data.publisher_id,
                                       timeout=180)
        ...
        connector_flow_id = flow_runner("connector",
                                       connector_id=test_data.connector_id,
                                       story_ids=test_data.news_item_ids,
                                       timeout=180)
        ...
        print(f"   Flows executed: bot -> presenter -> publisher -> connector")
