  ...
  >
    <template v-slot:content>
      <v-row
        ...
        class="valueHolder"
      >
        <span v-if="read_only || values[index].remote">{{
          values[index].value
        }}</span>
          ...
          <template v-slot:col_middle>
            ...
            <v-card class="text-center pb-3" color="white" outlined>
              <v-row justify="center">
                <v-col
                  v-for="metric in score.all"
                  :key="metric.name"
                  class="pa-0 mr-1 severity"
                  :class="metric.severity"
                  style="width: calc(100% / 3); border-radius: 4px"
                >
...
export default {
    ...
    rules: {
      vector: (value) => {
        ...
        return (
          value === '' ||
          pattern.test(value) ||
          'Invalid or Incomplete Vector String'
        )
      }
    }
  }),
  computed: {
    values() {
      return this.attribute_group.values
    }
  },
