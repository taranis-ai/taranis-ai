    ...
    <v-row v-bind="UI.TOOLBAR.ROW">
      <v-col class="py-0">
        ...
        <v-chip-group v-bind="UI.TOOLBAR.GROUP.DAYS">
          <v-chip
            v-bind="UI.TOOLBAR.CHIP.GROUP"
            v-for="day in days"
            :key="day.filter"
            @click="filterRange(day.filter)"
          >
            <div
              class="px-2"
              :title="$t('analyze.tooltip.range.' + day.filter)"
            >
              {{ $t(day.title) }}
            </div>
          </v-chip>
        ...
        <v-icon v-bind="UI.TOOLBAR.ICON.CHIPS_SEPARATOR">{{
          UI.ICON.SEPARATOR
        }}</v-icon>
        ...
        <v-chip-group v-bind="UI.TOOLBAR.GROUP.FAVORITES">
          <v-chip v-bind="UI.TOOLBAR.CHIP.GROUP" @click="filterCompleted">
            <v-icon
              v-bind="UI.TOOLBAR.ICON.FAVORITES_CHIP"
              :title="$t('analyze.tooltip.filter_completed')"
              >{{ UI.ICON.COMPLETED }}</v-icon
            >
          </v-chip>
        ...
        <v-chip-group v-bind="UI.TOOLBAR.GROUP.SORT">
          <v-chip
            v-bind="UI.TOOLBAR.CHIP.GROUP"
            @click="filterSort('DATE_DESC')"
            :title="$t('analyze.tooltip.sort.time.ascending')"
          >
            <v-icon v-bind="UI.TOOLBAR.ICON.SORT_CHIP_A">{{
              UI.ICON.CLOCK
            }}</v-icon>
            <v-icon v-bind="UI.TOOLBAR.ICON.SORT_CHIP_B">{{
              UI.ICON.DESC
            }}</v-icon>
          </v-chip>
          <v-chip
            v-bind="UI.TOOLBAR.CHIP.GROUP"
            @click="filterSort('DATE_ASC')"
...
export default {
  ...
  data: () => ({
    ...
    days: [
      {
        title: 'toolbar_filter.all',
        icon: 'mdi-information-outline',
        type: 'info',
        filter: 'ALL'
      },
      {
        title: 'toolbar_filter.today',
        icon: 'mdi-calendar-today',
        type: 'info',
        filter: 'TODAY'
      },
      {
        title: 'toolbar_filter.this_week',
        icon: 'mdi-calendar-range',
        type: 'info',
        filter: 'WEEK'
      },
      {
        title: 'toolbar_filter.this_month',
