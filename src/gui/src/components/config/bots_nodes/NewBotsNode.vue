<template>
    <div>
        <v-btn v-if="canCreate" depressed small color="white--text ma-2 mt-3 mr-5" @click="addNode">
            ...
            <span class="subtitle-2">{{$t('bots_node.add_btn')}}</span>
        </v-btn>
...
<script>
    ...
    export default {
        ...
        computed: {
            canCreate() {
                return this.checkPermission(Permissions.CONFIG_BOTS_NODE_CREATE)
            },
            canUpdate() {
                return this.checkPermission(Permissions.CONFIG_BOTS_NODE_UPDATE) || !this.edit
            },
        },
        methods: {
            addNode() {
                ...
                this.$validator.validateAll().then(() => {
                    ...
                    if (!this.$validator.errors.any()) {
                        ...
                        if (this.edit) {
                            ...
                            createNewBotsNode(this.node).then(() => {
