<template>
    <div>
        <v-btn v-if="canCreate" depressed small color="white--text ma-2 mt-3 mr-5" @click="addNode">
            ...
            <span class="subtitle-2">{{$t('collectors_node.add_btn')}}</span>
        </v-btn>
            ...
            <v-dialog v-model="visible" max-width="600" persistent>
                <v-card>
                    ...
                    <v-toolbar dark color="primary">
                        ...
                        <v-toolbar-title v-if="!edit">{{$t('collectors_node.add_new')}}</v-toolbar-title>
                        <v-toolbar-title v-if="edit">{{$t('collectors_node.edit')}}</v-toolbar-title>
                        <v-spacer></v-spacer>
                        <v-btn v-if="canUpdate" text type="submit" form="form">
                            <v-icon left>mdi-content-save</v-icon>
                            <span>{{$t('collectors_node.save')}}</span>
                        </v-btn>
                    </v-toolbar>
                    ...
                    <v-card-text>
                        <v-form @submit.prevent="add" id="form" ref="form">
                            ...
                            <v-textarea :disabled="!canUpdate"
                                    :label="$t('collectors_node.description')"
                                    name="description"
                                    v-model="node.description"
                                    :spellcheck="$store.state.settings.spellcheck"
                            ></v-textarea>
                            <v-text-field :disabled="!canUpdate"
                                    :label="$t('collectors_node.url')"
                                    name="url"
                                    type="text"
