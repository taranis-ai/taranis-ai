...
class StoryView(BaseView):
    ...
    def get_report_dialog(cls) -> tuple[str, int]:
        ...
        target = "#bookmark-detail-container" if bookmark_id else "#assess"
        if not bookmark_id and StoryView._get_current_url_path() != url_for("assess.assess"):
    ...
    def submit_report_dialog(cls) -> ResponseReturnValue:
        ...
        if request.form.get("bookmark_id") or StoryView._get_current_url_path() == url_for("assess.assess"):
    ...
    def rerender_list(cls, notification: str | None = None):
        if bookmark_id := request.form.get("bookmark_id"):
    ...
    def delete_story(cls, story_id: str):
        ...
        if bookmark_id := request.args.get("bookmark_id"):
            from frontend.views.story_bookmark_views import StoryBookmarkView

            notification_html = cls.get_notification_from_response(core_response)
            StoryBookmarkView._invalidate_bookmark_cache(bookmark_id)
            return StoryBookmarkView._render_detail(bookmark_id, notification=notification_html)
