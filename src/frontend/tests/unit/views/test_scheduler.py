...
def test_admin_sidebar_badges_link_to_filtered_errors_without_changing_main_links(
    ...
    assert {"transition-all", "hover:scale-110", "hover:brightness-90"} <= set(source_error_link.classes)
    assert {"transition-all", "hover:scale-110", "hover:brightness-90"} <= set(bot_error_link.classes)
    assert source_error_link.xpath("./span")[0].text == "Show errors"
    assert bot_error_link.xpath("./span")[0].text == "Show errors"
