...
class TestEndToEndUser(PlaywrightHelpers):
    ...
    def test_user_assess(self, logged_in_page: Page, forward_console_and_page_errors, pre_seed_stories):
        ...
        def access_story():
            ...
            story.get_by_role("button", name="Mark unread").click()
            story.get_by_test_id("toggle-important").click()
            story.get_by_role("button", name="Remove important").click()
            ...
            page.get_by_role("textbox", name="Tag name").press("Tab")
            page.get_by_role("textbox", name="Tag type (e.g. misc, location").fill("tag value")
            page.get_by_role("button", name="Add attribute").click()
            page.get_by_role("textbox", name="Key (e.g. TLP)").nth(1).fill("attr")
            page.get_by_role("textbox", name="Key (e.g. TLP)").nth(1).press("Tab")
            page.get_by_role("textbox", name="Value").nth(1).fill("value attr")
            page.get_by_role("button", name="Add tag").click()
            page.get_by_role("textbox", name="Tag name").nth(1).fill("tag 2")
            page.get_by_role("textbox", name="Tag name").nth(1).press("Tab")
            page.get_by_role("textbox", name="Tag type (e.g. misc, location").nth(1).fill("value2")
            page.get_by_role("textbox", name="Tag type (e.g. misc, location").nth(1).press("ArrowLeft")
            page.get_by_role("textbox", name="Tag type (e.g. misc, location").nth(1).fill("value 2")
            ...
            expect(page.get_by_role("main")).to_contain_text("tag name")
            expect(page.get_by_role("main")).to_contain_text("tag 2")
            expect(page.get_by_test_id("story-title")).to_contain_text("Pharmaceutical Trade Secrets Theft by APT76 edited title")
            expect(page.get_by_role("main")).to_contain_text("Test analyst comment")
