...
class SummarizeStatsTest(unittest.TestCase):
    def test_load_page_rows_filters_and_sorts_page_metrics(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                [row["name"] for row in summary_rows],
                ["assess:ready", "dashboard:ready"],
            )

            markdown = write_outputs(summary_rows, markdown_path, json_path)

            self.assertIn("Sorted by `p95` descending", markdown)
            self.assertIn("| assess:ready | 2 | 1 |", markdown)
            self.assertTrue(markdown_path.exists())

            json_rows = json.loads(json_path.read_text(encoding="utf-8"))
            self.assertEqual(json_rows[0]["name"], "assess:ready")
            self.assertEqual(json_rows[1]["name"], "dashboard:ready")
